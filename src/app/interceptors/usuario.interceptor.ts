...
export class UsuarioInterceptor implements HttpInterceptor {
  ...
  private checkInactivity() {
    ...
    setInterval(() => {
      ...
      const timeDifference = (currentTime?.getTime() ?? 0) - (this.lastInteractionTime?.getTime() ?? 0);
      const maxInactiveTime = 10 * 60 * 1000; // 10 minutos en milisegundos
      console.log('aqui probandooi')
